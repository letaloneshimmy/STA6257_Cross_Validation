...
![Nested Cross-validation where K=5,2. Created by Author](NestCV_fig.png#center){#fig-NestCV}
...
# Subset model B
predictors_b <- data[, c("FurnacSlag", "FlyAsh",  "CoarseAggregate",  "FineAggregate")]
target_b <- data$Strength
data_combined_b <- data.frame(predictors_b, Strength = target_b)

# Split model B
set.seed(84)
trainIndex_b <- createDataPartition(data_combined_b$Strength, p = .8, list = FALSE, times = 1)
train_data_b <- data_combined_b[trainIndex_b,]
test_data_b <- data_combined_b[-trainIndex_b,]
...
# Measure of Error df Model A
Measure_of_Error <- c("RMSE","MAE", "R2" )
Model_A <- c(kfold_rmse, kfold_mae, kfold_r2)
# kfold_df <- data.frame(Measure_of_Error, Model_A) 
# kfold_df %>%  kable()
...
set.seed(84)
train_control_kfold <- trainControl(method = "cv", number = 10)
model_kfold_b <- train(Strength ~ FurnacSlag + FlyAsh + CoarseAggregate + FineAggregate,
                     data = train_data_b, 
                     method = "lm", 
                     trControl = train_control_kfold)
kfold_predictions_b <- predict(model_kfold_b, newdata = test_data_b) 
kfold_rmse_b <- calculate_rmse(test_data_b$Strength, kfold_predictions_b) %>% round(2)
kfold_mae_b <- calculate_mae(test_data_b$Strength, kfold_predictions_b) %>% round(2)
kfold_r2_b <- calculate_r2(test_data_b$Strength, kfold_predictions_b) %>% round(2)

# Measure of Error df Model B
Measure_of_Error <- c("RMSE","MAE", "R2" )
Model_B <- c(kfold_rmse_b, kfold_mae_b, kfold_r2_b)
kfold_df <- data.frame(Measure_of_Error,Model_A, Model_B) 
kfold_df %>%  kable()
...
Model_A <- c(loocv_rmse, loocv_mae, loocv_r2)
# loocv_df <- data.frame(Measure_of_Error, Model_A) 
# loocv_df %>%  kable()

# LOOCV model b
set.seed(84)
train_control_loocv <- trainControl(method = "LOOCV")
model_loocv_b <- train(Strength ~ FurnacSlag + FlyAsh + CoarseAggregate + FineAggregate,
                     data = train_data_b, 
                     method = "lm", 
                     trControl = train_control_loocv)
loocv_predictions_b <- predict(model_loocv_b, newdata = test_data_b)
loocv_rmse_b <- calculate_rmse(test_data_b$Strength, loocv_predictions_b) %>% round(2)
loocv_mae_b <- calculate_mae(test_data_b$Strength, loocv_predictions_b) %>% round(2)
loocv_r2_b <- calculate_r2(test_data_b$Strength, loocv_predictions_b) %>% round(2)

Model_B <- c(loocv_rmse_b, loocv_mae_b, loocv_r2_b)
loocv_df <- data.frame(Measure_of_Error,Model_A, Model_B) 
loocv_df %>%  kable()
...
inner_train_control <- trainControl(method = "cv", number = 2)
...
Model_A <- c(nested_cv_rmse, nested_cv_mae, nested_cv_r2)
# nested_cv_df <- data.frame(Measure_of_Error, Model_A) 
# nested_cv_df %>%  kable()

#############
# model b
nested_model_b <- function(data, indices) {
  train_data <- data[indices,]
  test_data <- data[-indices,]
  
  model <- train(Strength ~ FurnacSlag + FlyAsh + CoarseAggregate + FineAggregate,
                 data = train_data, 
                 method = "lm", 
                 trControl = inner_train_control)
  predictions <- predict(model, newdata = test_data)
...
nested_cv_indices_b <- createFolds(data_combined_b$Strength, k = 5, list = TRUE, returnTrain = TRUE)
nested_cv_results_b <- t(sapply(nested_cv_indices_b, nested_model_b, data = data_combined_b))
nested_cv_rmse_b <- mean(nested_cv_results_b[, 1]) %>% round(2)
nested_cv_mae_b <- mean(nested_cv_results_b[, 2]) %>% round(2)
nested_cv_r2_b <- mean(nested_cv_results_b[, 3]) %>% round(2)

Model_B <- c(nested_cv_rmse_b, nested_cv_mae_b, nested_cv_r2_b)
nested_cv_df <- data.frame(Measure_of_Error, Model_A, Model_B) 
nested_cv_df %>%  kable()
...
result_long_df <- combine(kfold_results, LOOCV_results, nested_results) %>% 
  select(Method,Measure_of_Error, Model_A,Model_B )

# Print long df
result_long_df %>% 
  arrange(Method, Measure_of_Error) %>% 
  mutate(Better_Predictor = case_when(Measure_of_Error %in% c("RMSE", "MAE") & Model_A < Model_B ~ "Model A",
                                      Measure_of_Error %in% c("R2")          & Model_A > Model_B ~ "Model A",
                                      TRUE ~ "Model B") 
         ) %>%
  mutate(Method = factor(Method, levels = c("LOOCV","10-Fold","Nested CV")),
         Measure_of_Error = factor(Measure_of_Error, levels = c("RMSE", "MAE", "R2")) 
         )  %>%
  kable()
```
...
```{r, warning=FALSE, echo=TRUE}
#| label: analysis_result_plot

# Pivot wider tidy up model A
# result_wide_df <- result_long_df %>% 
#   select(-Model_B) %>% 
#   pivot_wider(names_from = Measure_of_Error, values_from = Model_A) %>% 
#   arrange(desc(R2), RMSE, MAE)
# result_wide_df %>% kable()

# Summary Plot A
summary_plotA <- result_long_df %>% 
  ggplot(aes(x = Method, y = Model_A)) +
  geom_bar(stat="identity", color = "blue4", fill = "gray78") +
  facet_wrap(~ Measure_of_Error, scales = "free", ncol = 3) +
  labs(title = "Model A",
       y = "Measure of Error Result",
