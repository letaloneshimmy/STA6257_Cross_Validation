...
:::
  ...
  <!-- - Dataset is divided into K subsets, knows as **folds** -->
  <!-- - All folds contain roughly an equal number of observations -->
  
<!-- For the K-Fold Cross-validation (K-Fold) method, the dataset is divided into K subsets, knows as "folds". Each fold contain roughly an equal number of observations. The model is trained on K−1 folds and tested on the remaining holdout fold. Every fold is used as the test set once, and this process is repeated K times . -->
...
<!-- With this method, the model is trained n times, where each training set has n−1 observations, and the test set has 1 observation. -->
...
<!-- The training data becomes the inner loop which optimizes hyperparameters and the test data is the outside loop that assesses the model’s performance . \ -->
<!-- \ -->
...
for (i in 1:nrow(train_data)) {
  ...
  params <- list(objective = "regression", metric = "rmse", learning_rate = 0.1, num_leaves = 31, max_depth = -1)
  model <- lgb.train(params = params, data = dtrain, nrounds = 3, verbose = -1) ############################################ Change back to 100
