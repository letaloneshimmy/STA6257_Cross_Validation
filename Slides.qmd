...
**Generalizability**: How well predictive models created from a sample fit other samples from the same population.
...
The goal of CV is to:
  ...
  - **Overfitting**: When a model fits the the underlying patterns of the training data too well.
...
::: {.column width="50%"}
![Created by Author](LOOCV_fig.png#center){#fig-LOOCV}
:::
...
Comparison_df <- rbind(
  Regression_R <- cbind(result_long_df, c(replicate(length(result_long_df),"Linear Regression"))) %>% rename(Model=4),
...
# print Comparison_df
Comparison_df %>% pivot_wider(names_from = Model, values_from = Result_Value) %>% relocate(Method) %>% kable()
